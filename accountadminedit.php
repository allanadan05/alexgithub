...
$query="SELECT * FROM `myaccounttbl` WHERE ID='$ID' ";
         $data=mysqli_query($con, $query);
         while($result=mysqli_fetch_array($data)){

         @$_SESSION['tempusername']=$result['username'];
         @$_SESSION['tempemail']=$result['email'];
         @$_SESSION['tempfname']=$result['firstname'];
         @$_SESSION['templname']=$result['lastname'];
     }
}
?>
				...
				<div class="container">
					...
					<div class="row">
						...
						<div id="loginform" class="login-card">
							<h3 style="color:#ff3300"><center>UPDATE</center></h3>
				        <form class="form-signin" action="accountadmineditcheck.php" method="POST"><span class="reauth-email"> </span>

				        	<?php
					        if(isset($_GET['adminmsgsuccess'])){
					        echo '<div class="alert alert-success" id="message"> <center>' .$_GET['adminmsgsuccess'] .'</center></div>';
					        header('refresh: 5; url=admin.php');
					        }
					        if(isset($_GET['adminmsgerr'])){
					        echo '<div class="alert alert-danger" id="message"> <center>' .$_GET['adminmsgerr'] .'</center></div>';
					        }
				            ?>

				        	<label>Basic Information:  </label>
				            <input class="form-control inputLogin" type="text" placeholder="Firstname" name="firstname" value="<?php echo $_SESSION['tempfname'];  ?>"  required>

				            <input class="form-control inputLogin" type="text" required="" placeholder="Lastname" name="lastname" value="<?php echo $_SESSION['templname'];  ?>" required>

				            <label>Login Details:  </label>
				            <input class="form-control inputLogin" type="email" placeholder="Email" name="email"  value="<?php echo $_SESSION['tempemail'];  ?>" required>
				            ...
				            <button class="primary-btn" type="submit" name="adminupdate" >  <span><i class="fa fa-save"></i> </span>  SAVE  </button>
